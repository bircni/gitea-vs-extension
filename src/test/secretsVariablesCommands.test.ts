...
const baseDeps: SecretsVariablesCommandsDeps = {
  getCurrentRepo: vi.fn().mockReturnValue(),
  ...
  createOrUpdateSecret: vi.fn().mockResolvedValue(),
  deleteSecret: vi.fn().mockResolvedValue(),
  createVariable: vi.fn().mockResolvedValue(),
  updateVariable: vi.fn().mockResolvedValue(),
  deleteVariable: vi.fn().mockResolvedValue(),
  showInputBox: vi.fn().mockResolvedValue(),
  showWarningMessage: vi.fn().mockResolvedValue(),
  refreshSecrets: vi.fn().mockResolvedValue(),
  refreshVariables: vi.fn().mockResolvedValue(),
};

describe("refreshSecrets", () => {
  it("does nothing when repo cannot be resolved", async () => {
    await refreshSecrets(baseDeps);
...
describe("refreshVariables", () => {
  it("does nothing when repo cannot be resolved", async () => {
    await refreshVariables(baseDeps);
